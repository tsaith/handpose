...
bool Fusion::hasEstimate() const {
    bool ret = ((mInitState & MAG) || (mMode == FUSION_NOMAG)) &&
...
bool Fusion::checkInitComplete(int what, const vec3_t& d, float dT) {
    ...
    if (hasEstimate()) {
        ...
        if (mMode != FUSION_NOMAG) {
            vec3_t  tmp;
            tmp.x = mData[1].y*up.z - mData[1].z*up.y;
            tmp.y = mData[1].z*up.x - mData[1].x*up.z;
            tmp.z = mData[1].x*up.y - mData[1].y*up.x;
            east = tmp * (length(tmp));
...
void Fusion::predict(const vec3_t& w, float dT) {
    ...
    if (x0.w < 0)
        x0 = -x0;
    //printf("%s,%d: %1.3f, %1.3f, %1.3f, %1.3f, \n",__func__,__LINE__,x0.x,x0.y,x0.z,x0.w);
